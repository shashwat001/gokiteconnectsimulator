...
// MainTicker is a Kite connect ticker instance.
type MainTicker struct {
	Conn *websocket.Conn
...
func NewMainTicker(apiKey string, accessToken string) *MainTicker {
	ticker := &MainTicker{
		apiKey:              apiKey,
...
func (t *MainTicker) SetRootURL(u url.URL) {
	t.url = u
}

// SetAccessToken set access token.
func (t *MainTicker) SetAccessToken(aToken string) {
	t.accessToken = aToken
}

// SetConnectTimeout sets default timeout for initial connect handshake
func (t *MainTicker) SetConnectTimeout(val time.Duration) {
	t.connectTimeout = val
}

// SetAutoReconnect enable/disable auto reconnect.
func (t *MainTicker) SetAutoReconnect(val bool) {
	t.autoReconnect = val
}

// SetReconnectMaxDelay sets maximum auto reconnect delay.
func (t *MainTicker) SetReconnectMaxDelay(val time.Duration) error {
...
func (t *MainTicker) SetReconnectMaxRetries(val int) {
	t.reconnectMaxRetries = val
}

// OnConnect callback.
func (t *MainTicker) OnConnect(f func()) {
	t.callbacks.onConnect = f
}

// OnError callback.
func (t *MainTicker) OnError(f func(err error)) {
	t.callbacks.onError = f
}

// OnClose callback.
func (t *MainTicker) OnClose(f func(code int, reason string)) {
	t.callbacks.onClose = f
}

// OnMessage callback.
func (t *MainTicker) OnMessage(f func(messageType int, message []byte)) {
	t.callbacks.onMessage = f
}

// OnReconnect callback.
func (t *MainTicker) OnReconnect(f func(attempt int, delay time.Duration)) {
	t.callbacks.onReconnect = f
}

// OnNoReconnect callback.
func (t *MainTicker) OnNoReconnect(f func(attempt int)) {
	t.callbacks.onNoReconnect = f
}

// OnTick callback.
func (t *MainTicker) OnTick(f func(tick models.Tick)) {
	t.callbacks.onTick = f
}

// OnOrderUpdate callback.
func (t *MainTicker) OnOrderUpdate(f func(order models.Order)) {
...
// routine.
func (t *MainTicker) ServeWithContext(ctx context.Context) {
...
func (t *MainTicker) handleClose(code int, reason string) error {
	t.triggerClose(code, reason)
	return nil
}

// Trigger callback methods
func (t *MainTicker) triggerError(err error) {
	if t.callbacks.onError != nil {
		t.callbacks.onError(err)
	}
}

func (t *MainTicker) triggerClose(code int, reason string) {
	if t.callbacks.onClose != nil {
		t.callbacks.onClose(code, reason)
	}
}

func (t *MainTicker) triggerConnect() {
	if t.callbacks.onConnect != nil {
		t.callbacks.onConnect()
	}
}

func (t *MainTicker) triggerReconnect(attempt int, delay time.Duration) {
	if t.callbacks.onReconnect != nil {
		t.callbacks.onReconnect(attempt, delay)
	}
}

func (t *MainTicker) triggerNoReconnect(attempt int) {
	if t.callbacks.onNoReconnect != nil {
		t.callbacks.onNoReconnect(attempt)
	}
}

func (t *MainTicker) triggerMessage(messageType int, message []byte) {
	if t.callbacks.onMessage != nil {
		t.callbacks.onMessage(messageType, message)
	}
}

func (t *MainTicker) triggerTick(tick models.Tick) {
	if t.callbacks.onTick != nil {
		t.callbacks.onTick(tick)
	}
}

func (t *MainTicker) triggerOrderUpdate(order models.Order) {
...
func (t *MainTicker) checkConnection(ctx context.Context, wg *sync.WaitGroup) {
	defer wg.Done()
	for {
		select {
		case <-ctx.Done():
...
func (t *MainTicker) readMessage(ctx context.Context, wg *sync.WaitGroup) {
	defer wg.Done()
	for {
		select {
		case <-ctx.Done():
...
func (t *MainTicker) Subscribe(tokens []uint32) error {
	if len(tokens) == 0 {
		return nil
	}
...
func (t *MainTicker) Unsubscribe(tokens []uint32) error {
	if len(tokens) == 0 {
		return nil
	}
...
func (t *MainTicker) SetMode(mode Mode, tokens []uint32) error {
	if len(tokens) == 0 {
		return nil
	}
...
func (t *MainTicker) Resubscribe() error {
	var tokens []uint32
...
func (t *MainTicker) processTextMessage(inp []byte) {
	var msg message
...
func (t *MainTicker) parseBinary(inp []byte) ([]models.Tick, error) {
...
func (t *MainTicker) splitPackets(inp []byte) [][]byte {
	var pkts [][]byte
	if len(inp) < 2 {
		return pkts
	}
